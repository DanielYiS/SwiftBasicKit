...
/// 弧形进度条
class ZCircularProgressBar: UIView {
    ...
    /// 进度条颜色
    var progressColoar = "#6F37E7".color
    /// 最大进度值 在其他公共属性最后面设置前面设置的属性才有效
    var maxprogress: Double = 20 {
        didSet {
    ...
    /// 当前进度
    @IBInspectable var progress: Double = 20 {
        didSet {
    ...
    final func setProgress(_ pro: Double, animated anim: Bool = true, withDuration duration: Double = 1) {
        progress = pro
        //进度条动画
        CATransaction.begin()
