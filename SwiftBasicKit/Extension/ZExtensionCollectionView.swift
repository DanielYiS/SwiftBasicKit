...
extension UIScrollView {
    private struct AssociatedKey {
        ...
        static var bgviewKey = "bgviewKey"
        static var lbErrorKey = "lbErrorKey"
    }
    public var aiView: UIActivityIndicatorView {
        get {
            guard let value = objc_getAssociatedObject(self, &AssociatedKey.aiviewKey) as? UIActivityIndicatorView else {
    ...
    public var bgView: UIView {
        get {
            guard let value = objc_getAssociatedObject(self, &AssociatedKey.bgviewKey) as? UIView else {
                let view = UIView.init(frame: CGRect.zero, backgroundColor: .clear)
        ...
        set {
            objc_setAssociatedObject(self, &AssociatedKey.bgviewKey, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    public var lbError: UILabel {
        get {
            guard let value = objc_getAssociatedObject(self, &AssociatedKey.lbErrorKey) as? UILabel else {
                let view = UILabel.init(frame: CGRect.zero, backgroundColor: .clear)
        ...
        set {
            objc_setAssociatedObject(self, &AssociatedKey.lbErrorKey, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
}
extension UICollectionView {
    ...
    public final func endLoading(count: Int, nodata: String) {
        self.aiView.stopAnimating()
        self.aiView.isHidden = true
        self.lbError.text = nodata
        self.lbError.isHidden = count > 0
    }
}
