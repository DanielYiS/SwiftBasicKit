...
/// 本地缓存数据管理
class ZLocalCacheManager: NSObject {
    
    /// 保存数据到本读指定文件路径
    static func func_setlocaldata(dic: [String: Any], key: String) {
        
        let filePath = ZLocalCacheManager.createLocalDataFolder().appendingPathComponent(key).appendingPathExtension("plist")
        do {
    ...
    /// 读取本读指定文件路径的数据
    static func func_getlocaldata(key: String) -> [String: Any]? {
