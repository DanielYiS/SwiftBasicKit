...
public class ZSettingApi: NSObject {
    ...
    /// 用户对象
    public var user: ZModelUserLogin?  {
    ...
    public static let shared = ZSettingApi()
    private var modelUser: ZModelUserLogin?
    ...
    public final func updatePhotos(array: [String]) {
        if array.count > 0 {
            var dic = [String: Any]()
            dic["photos"] = array
            dic["time"] = Date.init().timeIntervalSince1970
