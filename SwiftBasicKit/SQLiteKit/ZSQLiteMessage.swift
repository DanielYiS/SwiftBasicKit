    ...
    public static func getArrayMessageRecord<T: ZModelMessage>(models: inout [T]?, page: Int) {
        do {
            ...
            if ZSettingKit.shared.role == .anchor {
                let sql = "SELECT t1.*, (SELECT COUNT(*) FROM tb_message t3 WHERE t3.message_userid = t1.userid AND t3.message_read_state = 0) AS message_unread_count FROM tb_message_user t1 WHERE t1.login_userid = ? AND t1.role != ? GROUP BY t1.userid ORDER BY t1.role DESC, message_unread_count DESC, t1.message_time DESC LIMIT \(kPageCount) OFFSET \((page - 1) * kPageCount)"
                try ZSQLiteKit.shared.connection?.write({ db in
                    models = try T.fetchAll(db, sql: sql, arguments: [loginuserid, ZSettingKit.shared.role])
                })
            } else {
                let sql = "SELECT t1.*, (SELECT COUNT(*) FROM tb_message t3 WHERE t3.message_userid = t1.userid AND t3.message_read_state = 0) AS message_unread_count FROM tb_message_user t1 WHERE t1.login_userid = ? GROUP BY t1.userid ORDER BY t1.role DESC, message_unread_count DESC, t1.message_time DESC LIMIT \(kPageCount) OFFSET \((page - 1) * kPageCount)"
                try ZSQLiteKit.shared.connection?.write({ db in
